...
[토스증권 Open API](https://developers.tossinvest.com/docs) 연동(조회 전용)으로 실계좌 잔고·시세 조회 및 포트폴리오 동기화를 지원합니다 — [아래 섹션](#-토스증권-open-api-연동-조회-전용) 참고. (주문 실행은 미구현)
...
toss_client.py        # TossClient — OAuth2 토큰 + 계좌/보유/예수금/시세/호가/환율/캔들 조회
account_status.py     # 계좌 현황 CLI (잔고 + 보유종목 + 예수금)
prices.py             # 현재가 조회 CLI
order_preflight.py    # 주문 직전 점검 CLI (DRY-RUN, GO/NO-GO 판정)
toss_sync.py          # 토스 실계좌 → portfolio.json 동기화
```
...
토스증권 [Open API](https://developers.tossinvest.com/docs)로 **실계좌 잔고·시세를 조회**하고, 스캐너가 보던 `portfolio.json`을 실제 보유 포지션과 동기화합니다. OAuth2(Client Credentials) 인증이며, **국내(KRX) + 미국 주식**을 모두 지원합니다.

> **주문 실행(매수/매도)은 구현하지 않았습니다.** 조회·동기화·"주문 직전 점검"까지만 포함하며, 실제 `POST /orders` 호출은 의도적으로 제외했습니다.

```bash
# 환경 변수 (.env) — developers.tossinvest.com 에서 발급
TOSS_CLIENT_ID=...
TOSS_CLIENT_SECRET=...
# ⚠️ 발급 후 개발자 포털에 호출 IP 등록(allowlist) 필요

python account_status.py              # 계좌 잔고 + 보유종목 + 예수금
python prices.py 005930 AAPL TSLA     # 현재가 (국내코드/미국티커 혼용)
python order_preflight.py TSLA --side sell --qty 1   # 주문 직전 점검 (DRY-RUN)
python toss_sync.py                   # 실계좌 → portfolio.json 동기화 (미리보기)
python toss_sync.py --apply           # 실제 반영 (.bak 백업)
```

| 명령 | 역할 |
|---|---|
| `account_status.py` | 잔고·보유종목·평가손익·예수금 요약 |
| `prices.py` | 현재가 조회 (마켓 데이터) |
| `order_preflight.py` | 호가·상하한가·유의사항·장운영·수수료·환율 + 잔액/수량 검증 → GO/NO-GO (**주문은 안 함**) |
| `toss_sync.py` | 보유수량·평단을 실계좌로 교정. 스캐너 상태(전략/스톱) 보존, 국내종목은 `external_holdings.json`로 분리 |
